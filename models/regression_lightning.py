...
from utils import dataset_precip, dataset_hybrid
import argparse
import numpy as np

class node_base(pl.LightningModule):
    @staticmethod
...
class node_regression_base(node_base):
    @staticmethod
    def add_model_specific_args(parent_parser):
        parent_parser = node_base.add_model_specific_args(parent_parser)
        parser = argparse.ArgumentParser(parents=[parent_parser], add_help=False)
        parser.add_argument("--num_input_images", type=int, default=12)
        parser.add_argument("--num_output_images", type=int, default=6)
        parser.add_argument("--valid_size", type=float, default=0.1)
...
class Kriging_regression_base(node_base):
    @staticmethod
    def add_model_specific_args(parent_parser):
        parent_parser = node_base.add_model_specific_args(parent_parser)
        parser = argparse.ArgumentParser(parents=[parent_parser], add_help=False)
    ...
    def prepare_data(self):
        ...
        train_transform = None
        valid_transform = None
        precip_dataset = dataset_hybrid.precipitation_maps_h5_kriging
        self.train_dataset = precip_dataset(
            in_file=self.hparams.dataset_folder,
            num_input_images=self.hparams.num_input_images,
            num_output_images=self.hparams.num_output_images,
